##
## Info recordings Converter
## by mpiero azboxhd.es
##
##
...
class infoRecBm(Poll, Converter, object):
	...
	@cached
	def getText(self):
		assert self.type != self.PROGRESO
				...
				if timer.state == timer.StateRunning:
					if self.type == self.PROGRESO:
						...
						# if start_time <= now <= (start_time + duration) and duration > 0:
							# valor = (now - start_time) * 1000 / duration
						# else:
							# valor = 50	
						if (archivo in ref) or (ref==str(timer.service_ref)):
							# os.system("echo 'break'>>/tmp/testrec.txt")
